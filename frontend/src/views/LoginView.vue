...
const handleLogin = async () => {
  if (!email.value || !password.value) {
    // 2. API 调用方式改变，更简洁
    toast.error({ title: '错误', description: '邮箱和密码不能为空。' });
    return;
  }
  try {
    await authStore.login({ email: email.value, password: password.value });
    // 使用 toast.success 来显示成功状态
    toast.success({ title: '登录成功', description: '即将跳转到主面板...' });
  } catch (error) {
    // 使用 toast.error 来显示失败状态
    toast.error({ title: '登录失败', description: error.response?.data || '请检查您的邮箱和密码。' });
  }
};
</script>
